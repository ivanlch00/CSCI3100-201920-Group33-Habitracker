<?php
    ...
    if( !isset( $_SESSION['username']) ){
        ...
        if(isset($_POST['create_activity']) ){
            ...
            $activity_description = $_POST['goal_description'];
            $activity_subtask = $_POST['goal_subtask'];
            $username = $_SESSION['username'];
            $activity_enddate = date("Y-m-d", strtotime("+{$_POST['duration']} days"));
            $activity_starttime = "{$_POST['goal_starttime_hh']}:{$_POST['goal_starttime_mm']}:00";
            $activity_endtime = "{$_POST['goal_endtime_hh']}:{$_POST['goal_endtime_mm']}:00";
            $activity_public = (isset($_POST['goal_public'])) ? 1 : 0;
            //sanitize your input
            $activity_name = mysqli_real_escape_string($conn, $goal_name);
            $activity_description = mysqli_real_escape_string($conn, $goal_description);
            $activity_subtask = mysqli_real_escape_string($conn, $goal_subtask);
